...
**🔗 && 换行** — `bash_format.awk` 新增 `&&` 链检测：单行中出现 3 个及以上 `&&` 时，第 3 个起自动使用 ` \` 换行并缩进。`_and_count` 按行追踪 `&&` 计数；`;` 边界处计数重置。单个 `&`（后台）和 `&>`（重定向）正确忽略。

**📦 Box 边框动态宽度** — 将硬编码的 128 字符水平填充池替换为 `printf -v _h "%${_box_w}s" ""; _h="${_h// /─}"` 动态生成。修复宽终端（>128 列）下顶部/底部边框不对称的问题。

**🧹 `_strip_ansi` 简化** — `test_box_frame.sh`：将 bash `while` 循环 ANSI 剥离替换为单次 `sed 's/\x1b\[[0-9;]*m//g'` 管道，更简洁高效。

**🧪 新增测试** — `test_bash_format.sh` Phase 4：8 个 `&&` 换行测试（A01–A08）。`test_box_frame.sh` Phase 8：5 个宽终端边框一致性测试（wb1–wb5，60 组合 + 200 随机盒）。
